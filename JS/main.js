    ...
    if (confirm("Are you sure you want to delete this product?")) {
        lastDeleted = { product: items[i], index: i };
        items.splice(i, 1);
        save();
        drawList();
    ...
    allCards.addEventListener("mouseover", e => {
        ...
        if (cardInfo) hoveredIndex = [...allCards.children].indexOf(cardInfo);
    });
    allCards.addEventListener("mouseout", e => {
        const cardInfo = e.target.closest(".card-info");
        if (cardInfo) hoveredIndex = null;
    });
    ...
    document.addEventListener("keydown", function (event) {
        ...
        if (event.ctrlKey && event.key.toLowerCase() === "s") {
            event.preventDefault();
            if (hiddenIndex.value) form.requestSubmit();
        }

        if (event.key === "Delete") {
            event.preventDefault();
            if (hoveredIndex !== null) {
                deleteItem(hoveredIndex);
            } else {
                showToast("ضع المؤشر على المنتج المراد حذفه", "warning");
            }
        }
        ...
        document.addEventListener("keydown", function undoHandler(e) {
            if (e.ctrlKey && e.key.toLowerCase() === "z") {
            e.preventDefault();
            if (lastDeleted) {
                items.splice(lastDeleted.index, 0, lastDeleted.product);
                save();
                drawList();
                showToast(`<i class="fa-solid fa-rotate-left"></i> Undo Delete`, "info");
                lastDeleted = null;
            } else {
                showToast("No deletion to undo", "warning");
            }
